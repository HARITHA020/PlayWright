...
```ts
  ...
  6  |     const title=page.title()
  7  |     console.log("the title:",title);
  8  | 
  9  |     const expectvalue="40";
  10 |     const num1=page.locator('#sum1');
  11 |     await page.locator("#sum1").fill("20");
  12 |     console.log("the num 1 value:",num1.inputValue());
  13 |     const num2=page.locator('#sum2');
  14 |     await page.locator("#sum2").fill("20");
  15 |     console.log("the num 2 value:",num2.inputValue());
> 16 |     page.waitForTimeout(5000);
     |          ^ Error: page.waitForTimeout: Test ended.
  17 |     await page.getByRole('button',{name:'Get Sum'}).click();
  18 |     const result=await page.locator('#addmessage').textContent();
  19 |     console.log("the reuslt is:",result);
  20 |     await expect(page.locator("#addmessage")).toHaveValue(expectvalue);
  21 | });
```