...
```ts
  ...
  9  |     const sum1 = page.locator("#sum1");
  10 |     await sum1.scrollIntoViewIfNeeded();
  11 |     await sum1.fill("20");
  12 |     console.log("Num1:", await sum1.inputValue());
  13 |     const sum2 = page.locator("#sum2");
  14 |     await sum2.scrollIntoViewIfNeeded();
  15 |     await sum2.fill("20");
  16 |     console.log("Num2:", await sum2.inputValue());
  17 |     const getSumBtn = page.getByRole('button', { name: 'Get Sum' });
  18 |     await getSumBtn.scrollIntoViewIfNeeded();
  19 |     await getSumBtn.click();
  20 |     const result = page.locator("#addmessage");
  21 |     console.log("Result:", await result.textContent());
> 22 |     await expect(result).toHaveText("40");
     |                          ^ Error: expect(locator).toHaveText(expected) failed
  23 | });
```