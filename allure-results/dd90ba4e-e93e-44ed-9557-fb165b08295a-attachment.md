...
- Name: selinum playground.test.ts >> selenium playgroung
- Location: tests\selinum playground.test.ts:3:1

# Error details

```
Error: locator.innerText: Error: strict mode violation: locator('.border.border-gray-550.w-full.h-35.rounded.px-10') resolved to 3 elements:
...
Call log:
  - waiting for locator('.border.border-gray-550.w-full.h-35.rounded.px-10')
...
```ts
  ...
  2  | 
  3  | test('selenium playgroung', async ({ page }) => {
  4  |     await page.goto('https://www.testmuai.com/selenium-playground/simple-form-demo/')
> 5  |     const beforetext= await page.locator('.border.border-gray-550.w-full.h-35.rounded.px-10').innerText();
     |                                                                                               ^ Error: locator.innerText: Error: strict mode violation: locator('.border.border-gray-550.w-full.h-35.rounded.px-10') resolved to 3 elements:
  6  |     console.log("the before text:",beforetext);
  7  |     await page.locator('#user-message').fill("Haritha")
  8  |     await page.locator('showInput').click();
  9  |     const afterText=page.locator('#message')
  10 |     await expect(afterText).toHaveText("Haritha")
  11 | });
  12 | 
  13 | 
```