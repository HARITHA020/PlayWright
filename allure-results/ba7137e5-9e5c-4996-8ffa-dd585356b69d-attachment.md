...
```ts
  ...
  9  |     const formbox= await page.locator('gettotal')
> 10 |     await formbox.scrollIntoViewIfNeeded()
     |                   ^ Error: locator.scrollIntoViewIfNeeded: Test timeout of 30000ms exceeded.
  11 |     await page.locator("#sum1").fill("20");
  12 |     const num1=await page.locator('#sum1').inputValue();
  13 |     console.log("the num 1 value:",num1);
  14 |     await page.locator("#sum2").fill("20");
  15 |      const num2=await page.locator('#sum2').inputValue();
  16 |     console.log("the num 2 value:",num2);
  17 |     //await page.waitForTimeout(5000);
  18 |     const btn=await page.getByRole('button',{name:'Get Sum'}).click();
  19 |     
  20 |     const result=await page.locator('#addmessage').textContent();
  21 |     console.log("the reuslt is:",result);
  22 |     await expect(page.locator("#addmessage")).toHaveText(expectvalue);
  23 | });
```